...
namespace SostavSD.Pages.ProjectSostav
{
...

	{
        ...
        {
            _drawings.Clear();
            _drawings = await DrawingService.GetDrawingModelsByProjectIdAsync(ProjectId);
            return _drawings;
        }
        ...
        private async Task RemoveDrawings()
        {
            if (selectedItems.Count != 0)
            {
                ...
                {
                    await DrawingService.RemoveDrawingsAsync(item.DrawingId);
                }
        ...
        private async Task CopyDrawings()
        {
            ...
            if (_drawingsForCopy.Count > 0)
            {
                ...
                {
                    if (await DrawingService.AddSingleDrawingAsync(drawing) == 0)
                    {
